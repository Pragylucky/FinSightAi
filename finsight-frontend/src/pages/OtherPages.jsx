...
function HoldingRow({ holding, onClick }) {
  ...
  return (
    ...
    >
      ...
      <td className="py-3 pr-4 font-mono text-sm text-text-secondary">
        {quantity}
      </td>
...
export function Portfolio() {
  ...
  const totalCurrent = portfolio?.summary?.totalCurrent || 0;
  const totalPL = portfolio?.summary?.totalPL || 0;
  const totalPLPct = portfolio?.summary?.totalPLPercent || 0;
  ...
  return (
    <div className="space-y-5 page-enter">
      ...
      {/* Holdings Table */}
      <Card>
        <SectionHeader title="Holdings" subtitle={`${portfolio?.holdings?.length || 0} stocks`} />
    ...
    <div className="space-y-5 page-enter">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((item) => {
          ...
          const hitTarget =  item.currentPrice >= (item.targetPrice || 0) * 0.95;
          return (
              ...
              <div className="flex items-end justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-text-muted">
                    {item.targetPrice
                        ? ((item.targetPrice / item.currentPrice - 1) * 100).toFixed(1)
                        : "--"}% upside
                  </p>
                </div>
              </div>
...
export function News() {
  ...
  const categories = ['all', 'business', 'top news'];
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                <div className="flex-1">
                  ...
                  <h3
                    ...
                    className="font-semibold text-text-primary mb-2 leading-snug hover:text-accent-cyan cursor-pointer transition-colors"
                  >
                    {item.title}
                  </h3>
    ...
    const fetchScreenerStocks = async () => {
      try {
        ...
        const response = await fetch(
          'http://localhost:5000/api/screener?limit=20',
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
...
export function Funds() {
  ...
  const fetchMutualFunds = async () => {
    try {
      ...
      const response = await fetch(
        'http://localhost:5000/api/mutual-funds',
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      {filtered.map((fund) => (
        <Card key={fund.schemeCode || fund.id} hover>
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              {/* Badges */}
              <div className="flex items-center gap-2 mb-2">
                <Badge
                  variant={riskColors[fund.riskLevel] || 'default'}
                >
                  {fund.riskLevel}
                </Badge>
          ...
          {/* Fund details */}
          <div className="flex items-center justify-between pt-3 border-t border-bg-border text-xs">
            <div>
...
<div>
  <SectionHeader
    ...
    subtitle="Build a strong foundation before you invest"
  />

  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
    {investmentBasics.map((lesson) => (
      <Card
        key={lesson.id}
        className="cursor-pointer"
        onClick={() =>
          setOpenLesson(
            openLesson === lesson.id ? null : lesson.id
          )
        }
      >
        ...
        {openLesson === lesson.id && (
          <div className="mt-4 pt-3 border-t border-bg-border space-y-3">
